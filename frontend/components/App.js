...
export default function App({ lang = 'en' }) {
  ...
  return (
    <div>
      <h2>
        {text[language].TEXT_HEADING_CREATE_ACCOUNT}
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className="inputGroup">
          <label htmlFor="favFood">{text[language].LABEL_FAV_FOOD}</label>
          <select id="favFood" name="favFood" value={values.favFood} onChange={onChange}>
            <option value="">{text[language].TEXT_OPT_FAV_FOOD_1}</option>
            <option value="pizza">{text[language].TEXT_OPT_FAV_FOOD_2}</option>
            <option value="spaghetti">{text[language].TEXT_OPT_FAV_FOOD_3}</option>
            <option value="broccoli">{text[language].TEXT_OPT_FAV_FOOD_4}</option>
          </select>
        ...
        <div className="inputGroup">
          <label>
            ...
            {text[language].LABEL_ACCEPT_TERMS}
          </label>
          {errors.agreement && <div className="validation">{errors.agreement}</div>}
        </div>

        <div>
          <input type="submit" disabled={!submitAllowed} value={text[language].TEXT_SUBMIT} />
        </div>
      </form>
    </div>
  )
